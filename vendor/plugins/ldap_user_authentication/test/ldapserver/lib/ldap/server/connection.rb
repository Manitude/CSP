...
class Server
  ...
  class Connection
    ...
    def ber_read(io)
      ...
      # merged from https://github.com/wantmys2000/ruby-ldapserver/commit/738c931f38fb6d258ace85d4ab3a140bed4ae36b
    ...
    def abandon(messageID)
      @mutex.synchronize do
        ...
        thread.raise(LDAP::Abandon, 'LDAP::Abandon') if (thread && thread.alive?)
      end
    end
    ...
    def abandon_all
      ...
      @mutex.synchronize do
        @active_reqs.each do |id, thread|
          thread.raise(LDAP::Abandon, 'LDAP::Abandon') if thread.alive?
        end
        @active_reqs = {}
      end
    end

...
module HoptoadNotifier
  ...
  class << self
    ...
    def send_notice(notice)
      logger.error "\n\nException class " +  notice[:error_class].to_s + ' occurred at ' + Time.now.to_s
      logger.error notice[:error_message].to_s
      logger.error notice[:backtrace].lines.join("\n")
      
      if configuration.public?
        logger.info("Sending exception to hoptoad...")
        sender.send_to_hoptoad(notice.to_xml)
      else
         logger.info("Hoptoad notification suppressed for this configuration")
      end
    end

...
# Rosetta Stone local modification: prepend the location of our locally
# compiled extensions:
...
Dir.glob(File.expand_path("../precompiled/#{RUBY_PLATFORM}/**/rubyeventmachine.*", File.dirname(__FILE__))).each do |binary|
  begin
    next if succeeded
    require binary
    succeeded = true
  rescue LoadError => load_error
    latest_error = "Encountered error loading #{binary}: #{load_error}"
  end
end

if !succeeded
  $stderr.puts latest_error
  raise "Failed to load eventmachine binary for your platform (#{RUBY_PLATFORM}) Run ./compile_native_library.rb"
end

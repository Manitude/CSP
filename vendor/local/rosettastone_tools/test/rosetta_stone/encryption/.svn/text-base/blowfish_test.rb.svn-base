...
class RosettaStone::Encryption::BlowfishTest < Test::Unit::TestCase
  ...
  def test_encrypting_blank_value
    assert_raises(ArgumentError) do
      RosettaStone::Encryption::Blowfish.encrypt('', encryption_key)
    end
  end

  def test_encrypting_nil_value
    assert_raises(ArgumentError) do
      RosettaStone::Encryption::Blowfish.encrypt(nil, encryption_key)
    end
  end

  def test_decrypting_blank_value
    assert_raises(ArgumentError) do
      RosettaStone::Encryption::Blowfish.decrypt('', encryption_key)
    end
  end

  def test_decrypting_nil_value
    assert_raises(ArgumentError) do
      RosettaStone::Encryption::Blowfish.decrypt(nil, encryption_key)
    end
  end
...
private
  def assert_round_trip(test_message)
    ok_go = RosettaStone::Encryption::Blowfish.encrypt(test_message, encryption_key)
    back_on_it = RosettaStone::Encryption::Blowfish.decrypt(ok_go, encryption_key)
